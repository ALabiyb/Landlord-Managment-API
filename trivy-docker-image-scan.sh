...
dockerImageName=$(awk 'NR==14 {print $2}' Dockerfile)
...
docker run --rm aquasec/trivy -q image --exit-code 0 --severity HIGH --light $dockerImageName
docker run --rm aquasec/trivy -q image --exit-code 1 --severity CRITICAL --light $dockerImageName
