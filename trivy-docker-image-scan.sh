...
docker run --rm aquasec/trivy sbom -q image --exit-code 0 --severity HIGH --light $dockerImageName
docker run --rm aquasec/trivy sbom -q image --exit-code 1 --severity CRITICAL --light $dockerImageName
